...
if to == 'user' or service or is_admin1(msg) and to == "chat" or to == "channel" then
	if is_gbanned(msg.from.id) then
        return '<i>✨شما گولبال بن هستید✨.\n ✨برای ازاد کردن خود به www.raminsal.skynova.io یا ربات @raminsalofficial مراجعه کنید✨</i>'
	end
	...
	if matches[2]:lower() == 'english' and matches[3]:lower() == 'support' then
		...
		if is_banned(msg.from.id, tostring(target)) then
			return '<i>✨شما بن هستید.برای ازاد کردن خود بهwww.raminsal.skynova.io یا ربات @raminsalofficial مراجعه کنید✨</i>'
		end
	...
	elseif matches[2]:lower() == 'persian' and matches[3]:lower() == 'support' then
		...
		local long_id = data[tostring(target)]['long_id']
		if is_banned(msg.from.id, tostring(target)) then
			return '<i>✨شما بن هستید.برای ازاد کردن خود به www.raminsal.skynova.io یا ربات @raminsalofficial مراجعه کنید✨</i>'
		end
	...
	if matches[1] == 'help' and msg.to.type == 'user' or matches[1] == 'pmhelp' and is_admin1(msg) and msg.to.type ~= 'user' then
		...
		text = " <i>✨خوش آمدید✨!\n\n>برای دریافت گروه یا مدیریت گروه شما  یا ربات @raminsalofficial مراجعه کنید! یا بایکی از مدیران در تماس باشید, برای اینکار آیدی را چک کنید:\n[@Getandroid] \n\nبااستفاده از دستور 'راهنما' لیست دستورات را دریافت کنید!!\n\n✨باتشکر از انتخاب @Getandroid✨ </i>"
     	return text
    end
